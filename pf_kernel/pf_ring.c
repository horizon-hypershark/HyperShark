...
#ifdef PKAP_ENABLED
struct pf_ring_socket *pkap_pfr;
			   ...
			   int optlen)
{
  ...
  switch(optname) {
	...
	printk("ERROR: Can only be one PKAP Ring!\n");
	return -EINVAL;
	} else if (pkap_pfr && (pkap_pfr->pkap_ring == 1)) {
	printk("WARNING: This ring is already a PKAP Ring...\n");
	ret = 0;
	} else {
	pkap_pfr = pfr;
	...
	printk("Converted PF_RING to PKAP_RING\n");
	ret = 0;
	}
	break;
	#endif /* PKAP_ENABLED */
